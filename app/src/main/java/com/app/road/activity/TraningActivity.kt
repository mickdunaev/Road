...
class TraningActivity : AppCompatActivity() {
    ...
    private fun genegateCollectionName():String {
        val day = Repository.day.toString()
        return "finans_day" + day
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var collection = intent.getStringExtra("collection") ?: "finans_day1"
...
//            startActivity(intent)
    ...
    private fun genCollection(): String {
        var c = "finans_day1"
        val d = Repository.dayLena
        if(Repository.videoMode == 1){
            c = if(d < 5) "finansa1"
            else if(d < 9) "finansa2"
            else if(d < 13) "finansa3"
            else if(d < 17) "finansa4"
            else if(d < 21) "finansa5"
            else if(d < 25) "finansa6"
            else "finansa7"
        } else if(Repository.videoMode == 2){
            c = if(d < 5) "health1"
            else if(d < 9) "health2"
            else if(d < 13) "health3"
            else if(d < 17) "health4"
            else if(d < 21) "health5"
            else if(d < 25) "health6"
            else "health7"
        }
        return c
    }
    ...
    override fun onResume() {
        super.onResume()
        Utils.setScreenOpenAnalytics("Тренинг", this::class.java.simpleName)
    }
}